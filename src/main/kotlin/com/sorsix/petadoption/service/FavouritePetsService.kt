...
class FavouritePetsService(val petRepository: PetRepository, val userRepository: UserRepository) {
    ...
    fun likePet(petId: Long, username: String) {//ova treba da e id na momentalno najaveniot korisnik
    ...
    fun unlikePet(petId: Long, username: String) {//ova treba da e id na momentalno najaveniot korisnik
        val user = userRepository.findById(username).orElseThrow { InvalidUserIdException() }
        val pet = petRepository.findById(petId).orElseThrow { InvalidPetIdException() }
        user.unlikePet(pet)
        pet.unlike(user)
